...
define('WYT_SMTP_PASSWORD', 'your_real_mailbox_password');
define('WYT_SMTP_TIMEOUT_SECONDS', 20);
define('WYT_SMTP_EHLO_HOST', 'jasonjones.ninja');
```

Existing installations upgrading to version 2.0 must run the SQL migrations in
`sql/migrations/` before deploying the corresponding PHP code. The secure password
reset migration adds session-version tracking and request throttling, and revokes
tokens created by the earlier browser-visible reset flow.
